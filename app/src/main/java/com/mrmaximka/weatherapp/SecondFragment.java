...
public class SecondFragment extends Fragment {
    ...
    private void initViews(View view) {
        ...
        if (SecondActivity.getCityName() == null){  // Если переданы координаты
            requestRetrofit(null, SecondActivity.getLatitude(), SecondActivity.getLongitude());
        }
        else {      // Если передан город
            requestRetrofit(eng_towns[SecondActivity.getCityPosition()], null, null);
        }
    ...
    private void requestRetrofit(String city, String logitude, String latitude){
        Call<WeatherRequestRestModel> weatherRepo;
        if (city == null){  // Запрос по координатам
            weatherRepo = WeatherCoordRepo.getSingleton().getAPI().loadCoordWeather(logitude, latitude, API_KEY, "metric");
        }
        else {      // Запрос по названию города
            weatherRepo = WeatherRepo.getSingleton().getAPI().loadWeather(city, API_KEY, "metric");
        }
        weatherRepo.enqueue(new Callback<WeatherRequestRestModel>() {
                    @Override
